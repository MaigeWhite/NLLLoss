...
[[maybe_unused]] uint32_t NNOPBASE_NLLLoss = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnNLLLossTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "NLLLoss";
    char inputDesc[] = {1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {0, 0};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(x);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(target);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, target, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, weight, 2));
    if (reductionOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(reductionOptional), strlen(reductionOptional) + 1, 0, kNnopbaseString));
    } else {
        static char *reductionOptionalDef = "mean";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(reductionOptionalDef), strlen(reductionOptionalDef) + 1, 0, kNnopbaseString));
    }
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&ignoreIndexOptional), sizeof(int64_t), 1, kNnopbaseInt));
    ...
    if (NnopbaseAddParamName != NULL) {
        ...
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "target", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "weight", true));

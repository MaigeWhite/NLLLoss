...
def _build_args(x_in__, target_in__, weight_in__, y_out_, reduction, ignore_index):
    __inputs__ = []
    for arg in [x_in__, target_in__, weight_in__]:
        if arg != None:
...
@tbe_register.register_operator("NLLLoss", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def nll_loss(x_in__, target_in__, weight_in__, y_out_, reduction="mean", ignore_index=-100, kernel_name="nll_loss", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, target_in__, weight_in__, y_out_, reduction, ignore_index)
    ...
    origin_func_name = "nll_loss"
    ascendc_src_dir_ex = "nll_loss"
    ascendc_src_dir = "n_l_l_loss"
    ascendc_src_file = "nll_loss.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator NLLLoss. kernel name is " + kernel_name)
    op_type = "NLLLoss"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, target_in__, weight_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, target_in__, weight_in__, y_out_, reduction="mean", ignore_index=-100, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, target_in__, weight_in__, y_out_, reduction, ignore_index)
    result = check_op_cap("op_select_format", "NLLLoss", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, target_in__, weight_in__, y_out_, reduction="mean", ignore_index=-100, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, target_in__, weight_in__, y_out_, reduction, ignore_index)
    result = check_op_cap("get_op_specific_info", "NLLLoss", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
